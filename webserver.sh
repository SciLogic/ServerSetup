...
yes | sudo apt-get update -y >/dev/null && sudo apt-get upgrade >/dev/null && echo "$Green [OK] $Color_Off"

## Install AMP
echo "$Green \n Installing Apache2... $Color_Off"
yes | sudo apt-get install apache2 apache2-doc apache2-utils apache2-dev libexpat1 ssl-cert -y >/dev/null && echo "$Green [OK] $Color_Off"

echo "$Green \n Installing PHP & Reqs.... $Color_Off"
yes | sudo apt-get install libapache2-mod-php7.0 php7.0 php7.0-common php7.0-curl php7.0-dev php7.0-gd php-pear php-imagick php7.0-mcrypt php7.0-mysql >/dev/null && echo "$Green [OK] $Color_Off"

echo "$Green \n Installing MySQL.... $Color_Off"
yes | sudo apt-get install mysql-server mysql-client -y >/dev/null && echo "$Green [OK] $Color_Off"

echo "$Green \n Installing phpMyAdmin....$Color_Off"
yes | sudo apt-get install phpmyadmin -y >/dev/null && echo "$Green [OK] $Color_Off"

echo "$Green \n Verifying installs.... $Color_Off"
yes | sudo apt-get install apache2 libapache2-mod-php7.0 php7.0 mysql-server php-pear php7.0-mysql mysql-client mysql-server php-mysql php7.0-gd -y >/dev/null && echo "$Green [OK] $Color_Off"
